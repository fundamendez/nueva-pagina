...
function buildEmbedUrl(v: VideoItem): string {
  if (v.source === "drive") {
...
function buildOpenUrl(v: VideoItem): string {
  if (v.source === "drive") {
...
function buildDownloadUrl(v: VideoItem): string | null {
...
export default function ClasesGrabadas() {
  ...
  const renderGrid = (items: VideoItem[], section: "clases" | "tutoriales") => (
    <div className={styles.grid}>
      {items.map((v) => {
        const embedUrl = buildEmbedUrl(v);
        const openUrl = buildOpenUrl(v);
        const downloadUrl = buildDownloadUrl(v);
        const detailHref = useBaseUrl(`/ver-clases-grabadas?sec=${section}&t=${encodeURIComponent(v.title)}`);
          ...
          <section key={`${section}-${v.title}`} className={styles.card}>
            ...
            <div className={styles.actions}>
              <Link
                ...
                to={detailHref}
                state={{ title: v.title, embedUrl, openUrl, downloadUrl }}
              >
                Ver
              </Link>
    ...
    <Layout title="Clases Grabadas">
      <main className={styles.container} style={{ backgroundImage: bg }}>
        ...
        <div className={styles.overlay}>
          ...
          {renderGrid(tutoriales, "tutoriales")}
        </div>
      </main>
    </Layout>
  );
